...
export const update = async (channel: TextChannel | VoiceChannel, name: string | null, topic: string | null, nsfw: boolean | null) => {
...
export const assign = async (guildId: string, channel: TextChannel, category: MessageCategories, values: string[]) => {
    const $set = {
        ...
        ...(category === 'LEADERBOARDS_MESSAGE' && values.includes('Steam') && { 'leaderboards.steam.channelId': channel.id })
    }

    await settingsModel.updateOne({ guildId }, { $set });
}

export const dissociate = async (guildId: string, channel: TextChannel, category: MessageCategories, values: string[]) => {
    ...
    const isValid =
        ...
        category === 'LEADERBOARDS_MESSAGE' && values.includes('Steam') && settings?.leaderboards.steam.channelId === channel.id
    ...
    const $set = {
        ...
        ...(category === 'LEADERBOARDS_MESSAGE' && values.includes('Steam') && { 'leaderboards.steam.channelId': null })
    }
