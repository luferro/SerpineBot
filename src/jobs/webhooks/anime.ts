...
export const execute = async (client: Bot) => {
    ...
    const { 0: { data: { title, url, selftext } } } = await Reddit.getPostsByFlair('Anime', 'new', ['Episode']);
        
    const hasEntry = await client.manageState('Anime', 'Episodes', title, url);
    if(hasEntry) return;

    const streams = streamsList.map(item => {
    ...
    const aggregators = aggregatorsList.map(item => {
