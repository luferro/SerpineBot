...
export const execute = async (client: Bot) => {
    ...
    for(const subreddit of subreddits) {
        const { 0: { data: { title, permalink, url, secure_media, gallery_data } } } = await Reddit.getPosts(subreddit);
        ...
        for(const [guildId, guild] of client.guilds.cache) {
            ...
            if(secure_media || hasVideoExtension) {
                await webhook.send({ content: `**[${StringUtil.truncate(title)}](<https://www.reddit.com${permalink}>)**\n${nsfwUrl}` });
                continue;
            }
