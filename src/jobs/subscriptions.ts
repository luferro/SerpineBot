...
const getGamePass = async (page: Page, category: GamePassCategories) => {
    ...
    logger.info(`Subscriptions job found \`${items.length}\` items for \`${category} Game Pass\`.`);
}
...
const getUbisoftPlus = async (page: Page) => {
    ...
    logger.info(`Subscriptions job found \`${items.length}\` items for \`Ubisoft+\`.`);
}

const getEAPlay = async (page: Page, category: EAPlayCategories) => {
    await page.goto(`https://www.origin.com/irl/en-us/store/browse?fq=subscriptionGroup:${category === 'Pro' ? 'premium-' : ''}vault-games`);
    await page.waitForLoadState('networkidle');
    await page.waitForTimeout(5000);
    ...
    logger.info(`Subscriptions job found \`${items.length}\` items for \`${category === 'Pro' ? 'EA Play Pro' : 'EA Play'}\`.`);
}