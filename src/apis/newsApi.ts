...
const getNationalNews = async () => {
    const data = await fetch<Results<Article>>(`https://newsapi.org/v2/top-headlines?country=pt&pageSize=10&apiKey=${process.env.NEWS_API_KEY}`);
    return data.articles;
}

const getInternationalNews = async () => {
    const data = await fetch<Results<Article>>(`https://newsapi.org/v2/top-headlines?language=en&pageSize=10&apiKey=${process.env.NEWS_API_KEY}`);
    return data.articles;
}
