...
export const getDealById = async (id: string) => {
    ...
    const officialStores = await Promise.all(
        $('#official-stores .game-deals-container .game-deals-item').get().map(async (element) => {
            const store = $(element).children('.shop-image').find('img').first().attr('alt');
            const url = await UrlUtil.getRedirectLocation(`https://gg.deals${$(element).children('a').first().attr('href')}`);
    ...
    const keyshops = await Promise.all(
        $('#keyshops .game-deals-container .game-deals-item').get().map(async (element) => {
            const store = $(element).children('.shop-image').find('img').first().attr('alt');
            const url = await UrlUtil.getRedirectLocation(`https://gg.deals${$(element).children('a').first().attr('href')}`);
            const price = $(element).children('.game-info-wrapper').find('.game-price-current').first().text();
            const couponText = $(element).children('.game-info-wrapper').find('span.code').first().text();
...
export const getLatestBlogNews = async (category: BlogCategories) => {
    ...
    const image = $('.news-section .news-list .news-image-wrapper img').first().attr('src')?.replace('352cr184', '912cr476');
...
export const getLatestDeals = async (category: DealCategories) => {
    ...
    return Promise.all(
        $('.list-items > div').get().map(async (element) => {
            ...
            const isDataUriScheme = Boolean(image?.match(/^data:([A-Za-z-+\/]+);base64,(.+)$/));
            ...
            return {
                ...
                discounted,
                discount,
                image: !isDataUriScheme ? image : undefined
            }
        })
    );
}