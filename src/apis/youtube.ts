...
export const getChannelId = async (url: string) => {
    ...
    const data = await fetch<Video>(`https://youtube.googleapis.com/youtube/v3/videos?part=snippet&id=${videoId}&key=${process.env.YOUTUBE_API_KEY}`);
...
export const getSubscribers = async (url: string) => {
    ...
    const data = await fetch<Channel>(`https://youtube.googleapis.com/youtube/v3/channels?part=statistics&${queryOption}=${channelId}&key=${process.env.YOUTUBE_API_KEY}`);
