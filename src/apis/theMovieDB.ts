...
export const search = async (title: string, category: TheMovieDBCategories) => {
    const data = await fetch<Results<Result>>(`https://api.themoviedb.org/3/search/${category}?query=${title}&api_key=${process.env.THEMOVIEDB_API_KEY}`);
    return data.results[0]?.id.toString();
}

export const getMovieById = async (id: string) => {
    const data = await fetch<Movie>(`https://api.themoviedb.org/3/movie/${id}?api_key=${process.env.THEMOVIEDB_API_KEY}`);
...
export const getTvShowById = async (id: string) => {
    ...
    const { name, tagline, overview, homepage, status, first_air_date, next_episode_to_air, seasons, poster_path, vote_average, episode_run_time, genres } = data;
...
export const getProviders = async (id: string, category: TheMovieDBCategories) => {
    const data = await fetch<Providers>(`https://api.themoviedb.org/3/${category}/${id}/watch/providers?api_key=${process.env.THEMOVIEDB_API_KEY}`);
