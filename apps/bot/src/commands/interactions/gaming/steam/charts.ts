...
export const data: InteractionCommandData = new SlashCommandSubcommandBuilder()
	...
	.addIntegerOption((option) =>
		option
			...
			.addChoices(
				{ name: t('interactions.gaming.steam.charts.options.0.choices.0.name'), value: 1 },
				{ name: t('interactions.gaming.steam.charts.options.0.choices.1.name'), value: 2 },
				{ name: t('interactions.gaming.steam.charts.options.0.choices.2.name'), value: 3 },
			)
			.setRequired(true),
	);
	...
	const embed = new EmbedBuilder()
		.setTitle(
			t('interactions.gaming.steam.charts.embed.title', {
				chart: t(`interactions.gaming.steam.charts.options.0.choices.${chart - 1}.name`),
			}),
		)
