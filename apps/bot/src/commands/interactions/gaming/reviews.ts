...
export const execute: InteractionCommandExecute = async ({ client, interaction }) => {
	...
	const embed = new EmbedBuilder()
		...
		.addFields([
			{
				name: `**${t('gaming.reviews.embed.fields.0.name')}**`,
				value: releaseDate ?? t('common.unavailable'),
			},
			{
				name: `**${t('gaming.reviews.embed.fields.1.name')}**`,
				value: platforms.join('\n') || t('common.unavailable'),
			},
			{
				name: `**${t('gaming.reviews.embed.fields.2.name')}**`,
				value: score ?? t('common.unavailable'),
				inline: true,
			},
			{
				name: `**${t('gaming.reviews.embed.fields.3.name')}**`,
				value: count ?? t('common.unavailable'),
				inline: true,
			},
			{
				name: `**${t('gaming.reviews.embed.fields.4.name')}**`,
