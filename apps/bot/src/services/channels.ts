...
import { MessageCategory } from '@luferro/database';
...
export const unassignChannel = async (guildId: string, category: MessageCategory, channel: TextChannel) => {
	const message = await SettingsModel.getGuildMessage(guildId, category);
	if (message?.channelId !== channel.id) {
		throw new Error(`${MessageCategory[category]} message is not assigned to channel ${channel.name}.`);
	}
