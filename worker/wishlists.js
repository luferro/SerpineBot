...
const { formatCentsToEuros } = require('../utils/format');
    ...
    async checkWishlist(client) {
        try {
            ...
            for (const profile of profiles) {
                ...
                if(sale.length > 0) await this.sendNotification(client, profile.user, sale, 'sale');
                if(released.length > 0) await this.sendNotification(client, profile.user, released, 'released');
                if(subscriptions.added.length > 0) await this.sendNotification(client, profile.user, subscriptions.added, 'added');
                if(subscriptions.removed.length > 0) await this.sendNotification(client, profile.user, subscriptions.removed, 'removed');
            }
        } catch (error) {
